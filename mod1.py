...
#parameters#
n=100 ##sample size##
G=51 ##number of (equispaced) gridpoints##
a_l=0.4
b_l=0.6
h_grid=1/(G-1)
bandwidth=0.05
...
#functions#
def cov_scalar(s,t):
    phi_1_t=math.sqrt(5)*(6*t**2-6*t+1)
    phi_1_s=math.sqrt(5)*(6*s**2-6*s+1)
    if t<=0.5 and s<=0.5:
        phi_2_t=math.sqrt(2)*math.log(t+0.5)
        phi_2_s=math.sqrt(2)*math.log(s+0.5)
        phi_3_s=0.0
        phi_3_t=0.0
    elif t>0.5 and s<=0.5:
        phi_2_s=math.sqrt(2)*math.log(s+0.5)
        phi_2_t=0.0
        phi_3_t=math.sqrt(22)*(252*t**5-630*t**4+560*t**3-210*t**2+30*t-1)
        phi_3_s=0.0
    elif t>0.5 and s>0.5:
        phi_3_t=math.sqrt(22)*(252*t**5-630*t**4+560*t**3-210*t**2+30*t-1)
        phi_3_s=math.sqrt(22)*(252*s**5-630*s**4+560*s**3-210*s**2+30*s-1)
        phi_2_t=0.0
        phi_2_s=0.0
    else:
        phi_3_t=0.0
        ...
        phi_2_s=0.0
        phi_2_t=math.sqrt(2)*math.log(t+0.5)
    K=phi_1_s*phi_1_t+0.8*phi_2_s*phi_2_t+0.3*phi_3_s*phi_3_t
    return K
...
'testing the sum of squares for different bandwidths (<0.15) and choosing the optimal one'
